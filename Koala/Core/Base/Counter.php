...
abstract class Base_Counter{
	 ...
	 */
	abstract public function create($name,$value=0,$expires=-1);
	/**
	 * 移除一个计数器
	 * @param  string $name 计数器名称
	 * @return bool       true/false
	 ...
	 */
	abstract public function mget($list);
	/**
	 * 设置多个计数器的值
	 * 
	 * $keys格式
	 ...
	 */
	abstract public function mset($keys);
	/**
	 * 对计数器减
	 * @param  string $name 计数器名称
	 ...
	 * @return bool       计数器值/false
	 */
	abstract public function decrease($name,$value);
	/**
	 * 对计数器加
	 * @param  string $name 计数器名称
	 ...
	 * @return bool       计数器值/false
	 */
	abstract public function increase($name,$value);
	/**
	 * 获得所有计数器列表
	 * 返回数据格式
	 * array(
	 ...
	 */
	abstract public function getNums();
}