...
class Model {
	 ...
	 * @param array  $db_config  the database configuration
	 */
	public function __construct($name = '', $table_prefix = '', $db_config = array()) {
		//setting the model name
		switch (true) {
			case !empty($name):
				if (strpos($name, '.')) {// Support the definition of  {db_name.model_name}
					list($this->db_name, $this->name) = explode('.', $name);
				} else {
					$this->name = $name;
				}
				break;
		...
		switch (true) {
			case is_null($table_prefix):
				$this->table_prefix = '';
				break;
			case '' != $table_prefix:
				$this->table_prefix = $table_prefix;
				break;
			case !isset($this->table_prefix):
			default:
				$this->table_prefix = C('DB_PREFIX');
				break;
		}
		//database initialization
		$this->initDb(0, empty($this->db_config) ? $db_config : $this->db_config, true);
	}
	/**
	 * 切换当前的数据库连接
	 * @access public
	 ...
	 * @return Model
	 */
	public function initDb($linkNum = '', $config = '', $force = false) {
		if ('' === $linkNum && $this->db) {
			return $this->db;
		}
		static $_db = array();
		if (!isset($_db[$linkNum]) || $force) {
			...
			$_db[$linkNum] = \Koala\Server\Db::factory('', $config);
		} elseif (NULL === $config) {
	...
	protected function parseSql($sql, $parse) {
		...
		if (true === $parse) {
			...
			$sql = strtr($sql, array('__TABLE__' => $this->getTableName(), '__PREFIX__' => $this->table_prefix));
	...
	public function getTableName() {
		if (empty($this->real_table_name)) {
			$table_name = !empty($this->table_prefix) ? $this->table_prefix : '';
			if (!empty($this->table_name)) {
				$table_name .= $this->table_name;
			} else {
				$table_name .= parse_name($this->name);
			}
			$this->real_table_name = strtolower($table_name);
		}
		return (!empty($this->db_name) ? $this->db_name . '.' : '') . $this->real_table_name;
	}
	...
	public function table($table) {
		...
		if (is_array($table)) {
			...
			$table = preg_replace_callback("/__([A-Z_-]+)__/sU",
