...
class BCS{
	...
	public function __construct($ak = NULL, $sk = NULL, $hostname = NULL) {
		...
		if (! $ak && ! defined ( 'BCS_AK' ) && false === getenv ( 'HTTP_BAE_ENV_AK' )) {
			throw new BASException ( 'No account key was passed into the constructor.' );
		}
		if (! $sk && ! defined ( 'BCS_SK' ) && false === getenv ( 'HTTP_BAE_ENV_SK' )) {
			throw new BASException ( 'No secret key was passed into the constructor.' );
		}
		if ($ak && $sk) {
			$this->ak = $ak;
			$this->sk = $sk;
			...
			throw new BASException ( 'Construct can not get ak &sk pair, please check!' );
		}
		//valid $hostname 
		if (NULL !== $hostname) {
	...
	private function authenticate($opt) {
		...
		if (! ('/' == $opt [self::OBJECT] && '' == $opt [self::BUCKET] && 'GET' == $opt [self::METHOD] && ! isset ( $opt [self::QUERY_STRING] [self::ACL] )) && ! self::validate_bucket ( $opt [self::BUCKET] )) {
			throw new BASException ( $opt [self::BUCKET] . 'is not valid, please check!' );
		}
		//Validate object
		if (isset ( $opt [self::OBJECT] ) && ! self::validate_object ( $opt [self::OBJECT] )) {
			throw new BASException ( "Invalid object param[" . $opt [self::OBJECT] . "], please check.", - 1 );
		}
		//construct url
		$url = $this->format_url ( $opt );
		if ($url === false) {
			throw new BASException ( 'Can not format url, please check your param!', - 1 );
		}
		$opt ['url'] = $url;
		$this->log ( "[method:" . $opt [self::METHOD] . "][url:$url]", $opt );
		//build request
		$request = new RequestCore ( $opt ['url'] );
		$headers = array (
		...
		if (isset ( $opt ['fileUpload'] )) {
			if (! file_exists ( $opt ['fileUpload'] )) {
				throw new BASException ( 'File[' . $opt ['fileUpload'] . '] not found!', - 1 );
			}
			...
			if (isset ( $opt ["length"] )) {
				if ($opt ["length"] > $file_size) {
					throw new BASException ( "Input opt[length] invalid! It can not bigger than file-size", - 1 );
				}
				$length = $opt ['length'];
			}
			...
			if ($headers ['Content-Type'] === 'application/x-www-form-urlencoded') {
				...
				$mime_type = Utils_MimeTypes::get_mimetype ( $extension );
	...
	public function create_bucket($bucket, $acl = NULL, $opt = array()) {
		...
		if (NULL !== $acl) {
			if (! in_array ( $acl, self::$ACL_TYPES )) {
				throw new BASException ( "Invalid acl_type[" . $acl . "], please check!", - 1 );
			}
	...
	public function list_object($bucket, $opt = array()) {
		...
		if (empty ( $opt [self::BUCKET] )) {
			throw new BASException ( "Bucket should not be empty, please check", - 1 );
		}
		$opt [self::METHOD] = 'GET';
		$opt [self::OBJECT] = '/';
		$opt [self::QUERY_STRING] = array ();
	...
	public function create_object($bucket, $object, $file, $opt = array()) {
		...
		if (isset ( $opt ['acl'] )) {
			if (in_array ( $opt ['acl'], self::$ACL_TYPES )) {
				...
				throw new BASException ( "Invalid acl string, it should be acl_type", - 1 );
			}
			unset ( $opt ['acl'] );
		}
	 ...
	 * @param string $bucket (Required) 
	 * @param string $object (Required) 
	 * @param string $content (Required); 需要上传的文件的文件内容
	 * @param array $opt (Optional) 
	 * filename - Optional; 指定文件名
	 * acl - Optional ; 上传文件的acl，只能使用acl_type
	 * @return ResponseCore
	...
	public function create_object_by_content($bucket, $object, $content, $opt = array()) {
		...
		if ($content !== NULL && is_string ( $content )) {
			...
			throw new BASException ( "Invalid object content, please check.", - 1 );
		}
		if (isset ( $opt ['acl'] )) {
			if (in_array ( $opt ['acl'], self::$ACL_TYPES )) {
				self::set_header_into_opt ( "x-bs-acl", $opt ['acl'], $opt );
	...
	public function create_object_superfile($bucket, $object, $file, $opt = array()) {
		if (isset ( $opt ['length'] ) || isset ( $opt ['seekTo'] )) {
			throw new BASException ( "Temporary unsupport opt of length and seekTo of superfile.", - 1 );
		}
		//$opt array
		$this->assertParameterArray ( $opt );
		$opt [self::BUCKET] = $bucket;
		...
		//切片上传
		if (! file_exists ( $opt ['fileUpload'] )) {
			throw new BASException ( 'File not found!', - 1 );
		}
		...
		if (isset ( $opt ["sub_object_size"] )) {
			if (is_int ( $opt ["sub_object_size"] ) && $opt ["sub_object_size"] > 0) {
				...
				throw new BASException ( "Param [sub_object_size] invalid ,please check!", - 1 );
			}
		}
	...
	public function upload_directory($bucket, $dir, $opt = array()) {
		$this->assertParameterArray ( $opt );
		if (! is_dir ( $dir )) {
			throw new BASException ( "$dir is not a dir!", - 1 );
		}
		$result = array (
				"success" => 0, 
				"failed" => array (), 
		...
		if (isset ( $opt ["seek_object_id"] ) && isset ( $opt ["seek_object"] )) {
			throw new BASException ( "Can not set see_object_id and seek_object at the same time!", - 1 );
		}
		...
		if (isset ( $opt ["seek_object"] )) {
			if (isset ( $objects [$opt ["seek_object"]] )) {
				...
				throw new BASException ( "Can not find you seek object, please check!", - 1 );
			}
		}
		if (isset ( $opt ["seek_object_id"] )) {
			if (is_int ( $opt ["seek_object_id"] ) && $opt ["seek_object_id"] <= $objectCount) {
				...
				throw new BASException ( "Param seek_object_id not valid, please check!", - 1 );
			}
		}
		//上传objects
		$objectCount = count ( $objects );
	...
	public function copy_object($source, $dest, $opt = array()) {
		...
		if (empty ( $source ) || ! is_array ( $source ) || ! isset ( $source [self::BUCKET] ) || ! isset ( $source [self::OBJECT] )) {
			throw new BASException ( '$source invalid, please check!', - 1 );
		}
		if (empty ( $dest ) || ! is_array ( $dest ) || ! isset ( $dest [self::BUCKET] ) || ! isset ( $dest [self::OBJECT] ) || ! self::validate_bucket ( $dest [self::BUCKET] ) || ! self::validate_object ( $dest [self::OBJECT] )) {
			throw new BASException ( '$dest invalid, please check!', - 1 );
		}
	...
	public function set_object_meta($bucket, $object, $meta, $opt = array()) {
		...
		if (empty ( $meta )) {
			throw new BASException ( '$meta can not be empty! And $meta must be array.', - 1 );
		}
	 ...
	 * @param array $opt (Optional)
	 * @throws BASException
	 * @return boolean true|boolean false|ResponseCore
	 * true：object存在
	 * false：不存在
	 * ResponseCore其他错误
	 */
	...
	private static function set_header_into_opt($header, $value, &$opt) {
		if (isset ( $opt [self::HEADERS] )) {
			if (! is_array ( $opt [self::HEADERS] )) {
				...
				throw new BASException ( 'Invalid $opt[\'headers\'], please check.', - 1 );
			}
		} else {
	...
	private static function array_to_json($array) {
		if (! is_array ( $array )) {
			throw new BASException ( "Param must be array in function array_to_json()", - 1 );
		}
	...
	private function analyze_user_acl($acl) {
		...
		if (is_array ( $acl )) {
			...
			throw new BASException ( "Invalid acl.", - 1 );
		}
		return $result;
	}
	...
	private function check_user_acl($acl) {
		if (! is_array ( $acl )) {
			throw new BASException ( "Invalid acl array" );
		}
		foreach ( $acl ['statements'] as $key => $statement ) {
			...
			if (! isset ( $statement ['user'] ) || ! isset ( $statement ['resource'] ) || ! isset ( $statement ['action'] ) || ! isset ( $statement ['effect'] )) {
				throw new BASException ( 'Param miss: format acl error, please check your param!' );
			}
			if (! is_array ( $statement ['user'] ) || ! is_array ( $statement ['resource'] )) {
				throw new BASException ( 'Param error: user or resource must be array, please check your param!' );
			}
			if (! is_array ( $statement ['action'] ) || ! count ( array_diff ( $statement ['action'], self::$ACL_ACTIONS ) ) == 0) {
				throw new BASException ( 'Param error: action, please check your param!' );
			}
			if (! in_array ( $statement ['effect'], self::$ACL_EFFECTS )) {
				throw new BASException ( 'Param error: effect, please check your param!' );
			}
			if (isset ( $statement ['time'] )) {
				if (! is_array ( $statement ['time'] )) {
					throw new BASException ( 'Param error: time, please check your param!' );
				}
			}
		}
	...
	private function assertParameterArray($opt) {
		if (! is_array ( $opt )) {
			throw new BASException ( 'Parameter must be array, please check!', - 1 );
		}
	}
}