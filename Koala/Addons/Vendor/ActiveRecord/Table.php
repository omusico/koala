...
class Table
{
	...
	/**
	 * 加载table对象
	 * @param  string $model_class_name 模型名
	 * @param  string $tpr              表前缀
	 * @return object                   
	 */
	public static function load($model_class_name,$tpr='')
	{
		if (!isset(self::$cache[$model_class_name]))
		{
			...
			self::$cache[$model_class_name] = new Table($model_class_name,$tpr);
	...
	public function __construct($class_name,$tpr='')
	{	
		$this->tpr = $tpr;
	...
	private function set_table_name()
	{
		...
		$this->table = $this->tpr.$this->table;
